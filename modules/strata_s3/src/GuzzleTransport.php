...
/**
 * Carries a signed request to an endpoint over the HTTP client Drupal already ships.
 *
 * S3StorageProvider takes its transport as a callable so the whole provider can be tested against
 * canned responses with no network. This is the one that talks to a real endpoint.
 *
 ...
 * Guzzle's own exceptions are therefore unwrapped back into a status where they carry a response,
 * and only a genuine transport failure - a refused connection, a timeout, a DNS failure - surfaces
 * as a status of 0 with the reason in the body.
 */
final class GuzzleTransport
{
	 ...
	 *   Headers, including the Authorization the signer produced.
